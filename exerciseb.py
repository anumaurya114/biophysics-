...
# Making list or residue pairs to avoid repeated pairs
	...
	for rpair in sorted(respairs, key=lambda i: i[0].id[1]):
		...
		list5.append((resid1,rpair[0],resid2,rpair[1],eint,evdw,eint+evdw)) #sum of both energies
	
		#print (resid1,rpair[0].id[1],resid2,rpair[1].id[1],eint,evdw,eint+evdw) # eint= electrostatic, evdw= vanderwaals, eint+evdw= total
	i=0
	for_plot=[]
	for ele in sorted(list5, key=lambda li: li[6]): # Sort the seventh column and take the 5 first contacts
		i+=1
		print(ele)
		for at1 in ele[1].get_atoms():
			resid1 = rpair[0].get_resname()
			atid1 = at1.id
			print(atid1, resid1)
		for at2 in ele[3].get_atoms():
			resid1 = rpair[1].get_resname()
			atid2 = at2.id
			print(atid2, resid2)
